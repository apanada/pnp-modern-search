...
import { BaseWebComponent, IDataFilterValueInfo, ExtensibilityConstants, IDataFilterInfo, FilterConditionOperator, IDataFilterInternal } from '@pnp/modern-search-extensibility';
import * as ReactDOM from 'react-dom';
import { IComboBoxOption, Label, Icon, SelectableOptionMenuItemType, ComboBox, IComboBox, mergeStyles, Fabric, MessageBar, MessageBarType } from '@fluentui/react';
import { IReadonlyTheme } from '@microsoft/sp-component-base';
import update from 'immutability-helper';
import styles from './FilterKnowledgeRepositoryComponent.module.scss';
...
import { IKnowledgeRepositoriesFieldConfiguration } from '../../models/common/IKnowledgeRepositoriesProps';
...
export interface IFilterKnowledgeRepositoryComponentProps {
    ...
    knowledgeRepositories?: IKnowledgeRepositoriesFieldConfiguration[];
...
export interface IFilterKnowledgeRepositoryComponentState {
...
export class FilterKnowledgeRepositoryComponent extends React.Component<IFilterKnowledgeRepositoryComponentProps, IFilterKnowledgeRepositoryComponentState> {
    ...
    public constructor(props: IFilterKnowledgeRepositoryComponentProps) {
    ...
    public render() {
        ...
        let renderCombo: JSX.Element = <Fabric className={wrapperClassName}>
            <ComboBox
                ...
                placeholder={this.props.defaultOptions.length === 0 ? strings.Filters.FilterNoValuesMessage : strings.Filters.KnoledgeRepositoryComboBoxPlaceHolder}
        ...
        return <div className={styles.filterKnowledgeRepository}>
            {renderCombo}
            {renderIcon}
        </div>;
    }
    ...
    public componentDidMount() {
        ...
        const selectedValues: IDataFilterValueInfo[] = this.props.defaultOptions.map(option => {
            ...
            if (option.itemType === SelectableOptionMenuItemType.Normal && option.selected) {
                return {
                    ...
                    value: option.data.value as string,
        ...
        this.setState({
            ...
            selectedOptionKeys: selectedValues.map(v => v.name),
        ...
        if (option) {
            ...
            if (selectedValueIdx !== -1) {
                ...
                if (!option.selected && this.props.isMulti) {
                    ...
                    updatedSelectedValues = update(this.state.selectedValues, { [selectedValueIdx]: { selected: { $set: isSelected }, name: { $set: option.key as string } } });
                }
...
export class FilterKnowledgeRepositoryWebComponent extends BaseWebComponent {
    public constructor() {
        super();
    }
    ...
    public async connectedCallback() {
        ...
        let options: IComboBoxOption[] = [];
        let filterKnowledgeRepository: JSX.Element = null;
        let filter: IDataFilterInternal = null;

        if (props.filter) {

            const filter = props.filter as IDataFilterInternal;
            if (props.knowledgeRepositories && props.knowledgeRepositories.length > 0) {
                props.knowledgeRepositories.forEach((knowledgeRepo, key) => {
                    const selectedFilter = filter.values.filter(value => value.selected && value.name === knowledgeRepo.name);

                    options.push({
                        index: key,
                        key: knowledgeRepo.name,
                        text: knowledgeRepo.name,
                        itemType: SelectableOptionMenuItemType.Normal,
                        disabled: selectedFilter && selectedFilter.length > 0 ? selectedFilter[0].disabled : false,
                        selected: selectedFilter && selectedFilter.length > 0 ? selectedFilter[0].selected : false,
                        data: {
                            value: knowledgeRepo.url
                        },
                        styles: {
                            root: {
                                paddingTop: 8,
            ...
            filterKnowledgeRepository = <FilterKnowledgeRepositoryComponent
                {...props}
            ...
            filterKnowledgeRepository = <MessageBar
                messageBarType={MessageBarType.warning}
                isMultiline={false}>
                {`Component <pnp-knowledge-repository> misconfigured. The HTML attribute 'filter' is missing.`}
            </MessageBar>;
        }


        ReactDOM.render(filterKnowledgeRepository, this);
    }
}
